...
class Home(Screen):
    def __init__(self, **kwargs):
        ...
        title = Label(text="PCOD CARE APP", font_size=32, bold=True, size_hint_y=None, height=60)
...
class Period(Screen):
    def __init__(self, **kwargs):
        ...
        self.input = TextInput(hint_text="Last Period Date (DD-MM-YYYY)", size_hint_y=None, height=40)
        layout.add_widget(self.input)

        self.result = Label(text="", size_hint_y=None, height=30)
        layout.add_widget(self.result)

        save = Button(text="Save", size_hint_y=None, height=50)
...
class BMI(Screen):
    def __init__(self, **kwargs):
        ...
        self.w = TextInput(hint_text="Weight (kg)", input_filter='float', size_hint_y=None, height=40)
        self.h = TextInput(hint_text="Height (cm)", input_filter='float', size_hint_y=None, height=40)
        ...
        calc = Button(text="Calculate BMI", size_hint_y=None, height=50)
    ...
    def calc(self, instance):
        try:
            ...
            bmi_value = f"{bmi:.2f}"
            self.result.text = f"BMI: {bmi_value}"
            store.put("bmi", value=bmi_value)
        except:
...
class Risk(Screen):
    def __init__(self, **kwargs):
        ...
        self.q1 = TextInput(hint_text="Irregular periods? (yes/no)", size_hint_y=None, height=40)
        self.q2 = TextInput(hint_text="Weight gain? (yes/no)", size_hint_y=None, height=40)
        self.q3 = TextInput(hint_text="Acne / Hair growth? (yes/no)", size_hint_y=None, height=40)
        ...
        btn = Button(text="Check Risk", size_hint_y=None, height=50)
...
class Records(Screen):
    def __init__(self, **kwargs):
        ...
        self.label = Label(text="", font_size=18, size_hint_y=None)
        layout.add_widget(self.label)

        refresh = Button(text="Refresh", size_hint_y=None, height=50)
        refresh.bind(on_press=self.load_data)
    ...
    def load_data(self, instance):
        ...
        if store.exists("period"):
            text += f"Last Period: {store.get('period')['last_date']}\n"
        else:
            text += "Last Period: Not saved\n"

        if store.exists("bmi"):
            text += f"BMI: {store.get('bmi')['value']}\n"
        else:
            text += "BMI: Not calculated\n"
